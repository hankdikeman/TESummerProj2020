CatActivity <- function(sim_vals) {
  ...
  wtperc <- as.numeric(format(round(sim_vals[1,8]*39.997/(sim_vals[1,2]*885.4+sim_vals[1,5]*32.04)*100,2), nsmall = 2))
  ...
  CatAct <- ggplot(data = cat_df) +
    ...
    labs(title = "Normalized Catalyst Activity (0 to 1) as a Function of Time",
         subtitle = paste("Initial Catalyst Loading =", wtperc, "wt% NaOH")) +
    xlab("time (min)") +
